...
void CBlockPar::UnitDel(CBlockParUnit& el) {
    ...
    m_Units.remove_if([&el, &removed](auto& unit){
        bool result = &el == &unit;
        removed += result;
        return result;
    });
    ...
    if (removed > 1)
    {
        ERROR_S(L"Multiple units removed by UnitDel call");
    }
}
...
void CBlockPar::ParSetAdd(const std::wstring& name, const std::wstring& zn)
{
    ...
    auto res = std::find_if(m_Units.begin(), m_Units.end(), pred);
...
int CBlockPar::ParDelete(const std::wstring& name)
{
    DTRACE();

    size_t removed = 0;
    m_Units.remove_if([&name, &removed](const auto& u) {
        bool result = u.isPar() && (u.m_Name == name);
        removed += result;
        return result;
    });
...
int CBlockPar::ParCount(const std::wstring& name) const
{
    ...
    auto pred = [&name](const auto& u) { return u.isPar() && (u.m_Name == name); };
    return std::count_if(m_Units.begin(), m_Units.end(), pred);
}
